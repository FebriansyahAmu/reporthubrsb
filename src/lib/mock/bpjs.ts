...
/**
 * Fallback simulasi (dipakai saat SIMGOS belum dikonfigurasi). Menghasilkan
 * bentuk yang sama dengan data nyata lewat `buildAntreanResult`.
 */
export function getMockAntreanResult(f: AntreanFilter): AntreanResult {
  const tgl = f.tanggal || TODAY_STR;
  const scoped = ANTREAN.filter((a) => a.tanggal === tgl);
  return buildAntreanResult(scoped, { ...f, tanggal: tgl });
}
