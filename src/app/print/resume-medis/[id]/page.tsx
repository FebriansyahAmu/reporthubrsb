...
export default async function ResumeMedisPrintPage({
  ...
  if (!dto) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center px-6 text-center">
        ...
        <h1 className="text-lg font-semibold text-fg">Resume medis tidak ditemukan</h1>
  ...
  return (
    <>
      {/* Kertas F4 (215 × 330 mm) khusus dokumen ini. */}
      <style>{`@media print { @page { size: 215mm 330mm; margin: 12mm 12mm 14mm 12mm; } }`}</style>
      <PrintToolbar
        title={`Resume Medis · ${dto.pasien.nama}`}
        subtitle={dto.pasien.norm ?? undefined}
        backHref="/laporan/resume-medis"
      />
      <ResumeMedisDocument dto={dto} />
    </>
  );
}
