...
export default async function EdukasiFormPage({
  ...
  return (
    <FadeIn className="space-y-6">
      <BackLink nopen={nopen} />
      <PageHeader
        title="Edukasi Pasien & Keluarga Terintegrasi"
        description={`RM.21 · ${ctx.header.nama} · No. RM ${ctx.header.norm}`}
      />
      <EdukasiFormView nopen={nopen} header={ctx.header} initialSaved={ctx.saved} />
    </FadeIn>
  );
}
